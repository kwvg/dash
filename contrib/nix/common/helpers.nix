...
rec {
        ...
        src = pkgs.fetchurl {
          url =
            if info ? url then
              ...
              "https://files.pythonhosted.org/packages/source/${builtins.substring 0 1 name}/${name}/${name}-${info.version}.tar.gz";
          inherit (info) hash;
        };
        doCheck = false;
  ...
  mkCIEnv =
    ...
    pkgs.mkShellNoCC {
      ...
      buildInputs = [ compiler ] ++ packageLists.buildPackages ++ testPackages ++ extraBuildInputs;

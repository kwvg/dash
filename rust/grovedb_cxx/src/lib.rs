...
mod transaction;
use transaction::{
  BoxedTransaction, grovedb_commit_transaction, grovedb_rollback_transaction,
  grovedb_start_transaction,
};
...
mod query;
use query::{
  BoxedPathQuery, BoxedPathQueryVec,
...
use proof::{
  grovedb_prove_query,
  grovedb_verify_chained_queries, grovedb_verify_query,
...
pub(crate) mod ffi {
  ...
  /// Result of prove_query: the serialized proof bytes + operation costs.
  struct FfiProofResult {
    /// Bincode-encoded `GroveDBProof`.
    proof: Vec<u8>,
    cost: FfiOperationCost,
  }

  /// Result of verify_query variants: root hash + wire-encoded results.
  struct FfiVerifyResult {
    /// 32-byte Merkle root hash.
    root_hash: Vec<u8>,
    /// Wire-encoded `PathKeyOptionalElementTrio` entries:
    /// `[u32 count][path][key][has_elem][opt elem]…`
    results: Vec<u8>,
    /// Zero cost (verification does not mutate the database).
    cost: FfiOperationCost,
  }

  /// Batch apply options passed from C++ to control validation behavior.
  ...
  /// Result of verify_chained_queries: root hash + nested result sets.
  struct FfiChainedVerifyResult {
    /// Final 32-byte Merkle root hash.
    root_hash: Vec<u8>,
    /// Wire-encoded nested results:
    /// `[u32 query_count][result_set₁][result_set₂]…`
    /// Each result_set: `[u32 entry_count][entries…]`
    results: Vec<u8>,
    /// Zero cost (verification does not mutate the database).
    cost: FfiOperationCost,
  }
  ...
  extern "Rust" {
    ...
    // -- Chained verify --
    fn grovedb_verify_chained_queries(proof: &[u8], first_query: &BoxedPathQuery, chained_queries: &BoxedPathQueryVec) -> Result<FfiChainedVerifyResult>;

    // -- Batch --
    fn grovedb_apply_batch(db: &BoxedGroveDb, encoded_ops: &[u8], options: &FfiBatchApplyOptions) -> Result<FfiOperationCost>;
    fn grovedb_apply_batch_with_tx(db: &BoxedGroveDb, encoded_ops: &[u8], options: &FfiBatchApplyOptions, tx: &BoxedTransaction) -> Result<FfiOperationCost>;
    ...
    fn grovedb_find_subtrees(db: &BoxedGroveDb, path: &[u8]) -> Result<FfiOptionalBytesResult>;
    fn grovedb_find_subtrees_with_tx(db: &BoxedGroveDb, path: &[u8], tx: &BoxedTransaction) -> Result<FfiOptionalBytesResult>;

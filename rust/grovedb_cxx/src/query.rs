...
pub struct BoxedPathQuery {
  pub(crate) query: grovedb::PathQuery,
}
...
pub struct BoxedPathQueryVec {
  pub(crate) queries: Vec<grovedb::PathQuery>,
}
...
//   variant=0 Element:            [u32 elem_len][element bincode bytes]
//   variant=1 KeyElementPair:     [u32 key_len][key][u32 elem_len][element bytes]
//   variant=2 PathKeyElementTrio: [u32 seg_count][seg₁]…[u32 key_len][key][u32 elem_len][element bytes]
//
// Full result: [u32 count][u8 variant + data]…
...
pub(crate) fn encode_path_key_element_triples(

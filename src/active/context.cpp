...
ActiveContext::ActiveContext(CBLSWorker& bls_worker, ChainstateManager& chainman, CConnman& connman,
    ...
    qdkgsman{std::make_unique<llmq::CDKGSessionManager>(dmnman, qsnapman, chainman, sporkman, db_params,
                                                        quorums_watch)},
    shareman{std::make_unique<llmq::CSigSharesManager>(connman, chainman.ActiveChainstate(), sigman, peerman,
                                                       *nodeman, qman, sporkman)},
    ...
    qman_handler{std::make_unique<llmq::QuorumParticipant>(bls_worker, connman, dmnman, qman,
                                                           qsnapman, *nodeman, chainman, mn_sync,
                                                           sporkman, sync_map, quorums_recovery, quorums_watch)},
    cl_signer{std::make_unique<chainlock::ChainLockSigner>(chainman.ActiveChainstate(), clhandler,
                                                           sigman, *shareman, sporkman, mn_sync)},
    is_signer{std::make_unique<instantsend::InstantSendSigner>(chainman.ActiveChainstate(), clhandler,
                                                               isman, sigman, *shareman,
                                                               qman, sporkman, mempool, mn_sync)}
{
    // Setup CoinJoin server
    auto cj_server = std::make_unique<CCoinJoinServer>(&peerman, chainman, connman, dmnman, dstxman, mn_metaman,
                                                       mempool, *nodeman, mn_sync, isman);
    m_cj_server = cj_server.get();
    peerman.AddExtraHandler(std::move(cj_server));

    // Setup DKG session handler
    qdkgsman->InitializeHandlers([&](const Consensus::LLMQParams& llmq_params,
        ...
        return std::make_unique<llmq::ActiveDKGSessionHandler>(bls_worker, dmnman, mn_metaman, *dkgdbgman,
                                                               *qdkgsman, qblockman, qsnapman, *nodeman,
                                                               chainman, sporkman, llmq_params, quorums_watch,
                                                               quorum_idx);
    });

    // Setup pluggables for LLMQ

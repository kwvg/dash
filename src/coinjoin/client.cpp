...

    {
        ...
        if (dsq.fReady &&
            m_walletman.ForAnyCJClientMan([this, &connman, &dmn](std::unique_ptr<CCoinJoinClientManager>& clientman) {
                return clientman->TrySubmitDenominate(dmn->pdmnState->addr.GetPrimaryService(), connman);
            })) {
            LogPrint(BCLog::COINJOIN, "DSQUEUE -- CoinJoin queue (%s) is ready on masternode %s\n", dsq.ToString(),
                     dmn->pdmnState->addr.GetPrimaryService().ToStringAddrPort());
            return {};
        } else {
...
void CCoinJoinClientSession::ProcessMessage(CNode& peer, CChainState& active_chainstate, CConnman& connman, const CTxMemPool& mempool, std::string_view msg_type, CDataStream& vRecv)
{
    ...
    if (msg_type == NetMsgType::DSSTATUSUPDATE) {
        ...
        if (mixingMasternode->pdmnState->addr.GetPrimaryService() != peer.addr) {
            return;
        }
        ...
        if (mixingMasternode->pdmnState->addr.GetPrimaryService() != peer.addr) {
            WalletCJLogPrint(m_wallet, "DSCOMPLETE -- message doesn't match current Masternode: infoMixingMasternode=%s  addr=%s\n",
                             mixingMasternode->pdmnState->addr.GetPrimaryService().ToStringAddrPort(), peer.addr.ToStringAddrPort());
            return;
        }
...
bool CCoinJoinClientSession::JoinExistingQueue(CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    while (m_queueman->GetQueueItemAndTry(dsq)) {
        ...
        if (connman.IsMasternodeOrDisconnectRequested(dmn->pdmnState->addr.GetPrimaryService())) {
        ...
        pendingDsaRequest = CPendingDsaRequest(dmn->pdmnState->addr.GetPrimaryService(), CCoinJoinAccept(nSessionDenom, txMyCollateral));
    ...
    while (nTries < 10) {
        ...
        if (nLastDsq != 0 && nDsqThreshold > m_mn_metaman.GetDsqCount()) {
            WalletCJLogPrint(m_wallet, /* Continued */
                             ...
                             dmn->proTxHash.ToString(), dmn->pdmnState->addr.GetPrimaryService().ToStringAddrPort(), nLastDsq,
                             nDsqThreshold, m_mn_metaman.GetDsqCount());
            nTries++;
            continue;
        }
        ...
        WalletCJLogPrint(m_wallet, "CCoinJoinClientSession::StartNewQueue -- attempt %d connection to Masternode %s\n",
                         nTries, dmn->pdmnState->addr.GetPrimaryService().ToStringAddrPort());
        ...
        WalletCJLogPrint( /* Continued */
            ...
            nSessionDenom, CoinJoin::DenominationToString(nSessionDenom), dmn->pdmnState->addr.GetPrimaryService().ToStringAddrPort());
        strAutoDenomResult = _("Trying to connect…");
        return true;
    }
...
bool CCoinJoinClientManager::TrySubmitDenominate(const CService& mnAddr, CConnman& connman)
{
    ...
    for (auto& session : deqSessions) {
        ...
        if (session.GetMixingMasternodeInfo(mnMixing) && mnMixing->pdmnState->addr.GetPrimaryService() == mnAddr && session.GetState() == POOL_STATE_QUEUE) {
...
void CCoinJoinClientSession::RelayIn(const CCoinJoinEntry& entry, CConnman& connman) const
{
    ...
    connman.ForNode(mixingMasternode->pdmnState->addr.GetPrimaryService(), [&entry, &connman, this](CNode* pnode) {
...
void CCoinJoinClientSession::GetJsonInfo(UniValue& obj) const
{
    ...
    if (mixingMasternode != nullptr) {
        ...
        obj.pushKV("service", mixingMasternode->pdmnState->addr.GetPrimaryService().ToStringAddrPort());
    }

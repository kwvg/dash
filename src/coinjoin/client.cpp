...
//
bool CCoinJoinClientSession::DoAutomaticDenominating(CConnman& connman, CTxMemPool& mempool, CBlockPolicyEstimator& fee_estimator, bool fDryRun)
{
...
bool CCoinJoinClientManager::DoAutomaticDenominating(CConnman& connman, CTxMemPool& mempool, CBlockPolicyEstimator& fee_estimator, bool fDryRun)
{
    if (fMasternodeMode) return false; // no client-side mixing on masternodes
    if (!CCoinJoinClientOptions::IsEnabled() || !IsMixing()) return false;
    ...
    for (auto& session : deqSessions) {
        ...
        fResult &= session.DoAutomaticDenominating(connman, mempool, fee_estimator, fDryRun);
    }
...
bool CCoinJoinClientSession::MakeCollateralAmounts(CBlockPolicyEstimator& fee_estimator)
{
    if (!CCoinJoinClientOptions::IsEnabled()) return false;
...
bool CCoinJoinClientSession::MakeCollateralAmounts(CBlockPolicyEstimator& fee_estimator, const CompactTallyItem& tallyItem, bool fTryDenominated)
{
    AssertLockHeld(mixingWallet.cs_wallet);
...
void CCoinJoinClientManager::DoMaintenance(CConnman& connman, CTxMemPool& mempool, CBlockPolicyEstimator& fee_estimator)
{
    if (!CCoinJoinClientOptions::IsEnabled()) return;
    if (m_mn_sync == nullptr) return;
    if (fMasternodeMode) return; // no client-side mixing on masternodes
    ...
    if (nDoAutoNextRun == nTick) {
        DoAutomaticDenominating(connman, mempool, fee_estimator);
...
void DoCoinJoinMaintenance(CConnman& connman, CTxMemPool& mempool, CBlockPolicyEstimator& fee_estimator)
{
    ...
    for (const auto& pair : coinJoinClientManagers) {
        pair.second->DoMaintenance(connman, mempool, fee_estimator);
    }
}

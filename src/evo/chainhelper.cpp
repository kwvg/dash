...
CChainstateHelper::CChainstateHelper(CCreditPoolManager& cpoolman, CDeterministicMNManager& dmnman, CMNHFManager& mnhfman, CGovernanceManager& govman,
    ...
    : isman{isman}, clhandler{clhandler},
...
int32_t CChainstateHelper::GetBestChainLockHeight() const
{
    return clhandler.GetBestChainLock().getHeight();
}

/** Passthrough functions to CInstantSendManager */
std::optional<
    std::pair</*islock_hash=*/uint256, /*txid=*/uint256>
> CChainstateHelper::HasConflictingISLock(const CTransaction& tx) const
{
...
bool CChainstateHelper::IsInstantSendWaitingForTx(const uint256& hash) const
{
    return isman.IsWaitingForTx(hash);
}
...
bool CChainstateHelper::ShouldInstantSendRejectConflicts() const
{
    return isman.RejectConflictingBlocks();
}

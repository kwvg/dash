...
public:
    ...
    {
        return 0; // ranges::count_if(mnMap, [](const auto& p){ return IsMNValid(*p.second); });
    }

    [[nodiscard]] size_t GetAllHPMNsCount() const
    {
        return 0; // ranges::count_if(mnMap, [](const auto& p) { return p.second->nType == MnType::HighPerformance; });
    }

    [[nodiscard]] size_t GetValidHPMNsCount() const
    {
        return 0; // ranges::count_if(mnMap, [](const auto& p) { return p.second->nType == MnType::HighPerformance && IsMNValid(*p.second); });
    }

    [[nodiscard]] size_t GetValidWeightedMNsCount() const
    {
        return 0; /* std::accumulate(mnMap.begin(), mnMap.end(), 0, [](auto res, const auto& p) {

...
private:
    ...
    struct Member
    {
        ...
        static constexpr auto& get(CDeterministicMNState& state) { return state.*Field; }
        static constexpr const auto& get(const CDeterministicMNState& state) { return state.*Field; }
    };
    ...
    SERIALIZE_METHODS(CDeterministicMNStateDiff, obj)
    {
        ...
        boost::hana::for_each(members, [&](auto&& member) {
            ...
            if constexpr (BaseType::mask == Field_pubKeyOperator) {
                static_assert(std::is_same_v<T1, CBLSLazyPublicKey>, "pubKeyOperator has unexpected type (expected CBLSLazyPublicKey)");

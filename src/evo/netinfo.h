...
namespace {
inline uint8_t GetSupportedServiceType(const CService& service)
{
    if (service.IsIPv4()) {
...
inline bool IsSupportedServiceType(const uint8_t& type)
{
    switch (type) {
...
public:
    ...
    NetInfoEntry(const DomainPort& service) :
        m_type{Extensions::DOMAINS},
        m_data{service}
    {
    }
    NetInfoEntry(const CService& service) :
        m_type{GetSupportedServiceType(service)},
        m_data{service}
    {
    }
...
public:
    ...
    void Serialize(Stream& s) const
    {
        if (const auto* data_ptr{std::get_if<CService>(&m_addr.m_data)};
            data_ptr != nullptr && IsSupportedServiceType(m_addr.m_type)) {
            s << *data_ptr;
        } else {
            s << CService();
        }
    ...
    void Serialize(CSizeComputer& s) const { s.seek(::GetSerializeSize(CService{}, s.GetVersion())); }

...
bool ProcessSpecialTxsInBlock(const llmq::Context& ctx, const CBlock& block, const CBlockIndex* pindex, CValidationState& state, const CCoinsViewCache& view, bool fJustCheck, bool fCheckCbTxMerleRoots)
{
    AssertLockHeld(cs_main);
    ...
    try {
        ...
        if (fCheckCbTxMerleRoots && !CheckCbTxMerkleRoots(*ctx.quorumBlockProcessor, block, pindex, state, view)) {
            // pass the state returned by the function above
            return false;
        }

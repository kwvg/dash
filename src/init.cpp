    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                node.llmq_ctx.reset();
                node.llmq_ctx.reset(new LLMQContext(Assert(node.chainman)->ActiveChainstate(), *node.connman, *node.evodb, *node.mempool, *::sporkManager, node.peerman, false, fReset || fReindexChainState));

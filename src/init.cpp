...
bool AppInitMain(const CoreContext& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    node.mempool = std::make_unique<CTxMemPool>(&::feeEstimator, check_ratio);
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                chainman.InitializeChainstate(*Assert(node.mempool), llmq::chainLocksHandler, llmq::quorumInstantSendManager, llmq::quorumBlockProcessor, node.evodb);
    ...
    if (fMasternodeMode) {
        ...
        node.scheduler->scheduleEvery(std::bind(&DoCoinJoinMaintenance, std::ref(*node.connman), std::ref(*node.mempool), std::ref(::feeEstimator)), 1 * 1000);
#endif // ENABLE_WALLET
    }

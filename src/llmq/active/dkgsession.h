...
#ifndef BITCOIN_LLMQ_ACTIVE_DKGSESSION_H
#define BITCOIN_LLMQ_ACTIVE_DKGSESSION_H
...
public:
    ...
    void VerifyAndJustify(CDKGPendingMessages& pendingMessages, PeerManager& peerman) override EXCLUSIVE_LOCKS_REQUIRED(!invCs);
    void SendJustification(CDKGPendingMessages& pendingMessages, PeerManager& peerman, const std::set<uint256>& forMembers) override;
...
private:
    //! CDKGSession
    bool MaybeDecrypt(const CBLSIESMultiRecipientObjects<CBLSSecretKey>& obj, size_t idx, CBLSSecretKey& ret_obj, int version) override;
};
} // namespace dkg
} // namespace llmq

#endif // BITCOIN_LLMQ_ACTIVE_DKGSESSION_H

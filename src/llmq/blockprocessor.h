...
public:
    ...
    void ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, CQuorumManager& quorumManager);
...
private:
    ...
    bool ProcessCommitment(int nHeight, const uint256& blockHash, const CFinalCommitment& qc, CValidationState& state, const CQuorumManager& quorumManager, bool fJustCheck, bool fBLSChecks) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

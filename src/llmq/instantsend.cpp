...
bool CInstantSendManager::TrySignInputLocks(const CTransaction& tx, bool fRetroactive, Consensus::LLMQType llmqType, const Consensus::Params& params)
{
    ...
    for (const auto& in : tx.vin) {
        ...
        if (node.quorumSigningManager->GetVoteForId(params.llmqTypeDIP0024InstantSend, id, otherTxHash) ||
            node.quorumSigningManager->GetVoteForId(params.llmqTypeInstantSend, id, otherTxHash)) {
        ...
        if (auto llmqs = {params.llmqTypeDIP0024InstantSend, params.llmqTypeInstantSend};
            ranges::any_of(llmqs, [this, &id, &tx](const auto& llmqType){
                return node.quorumSigningManager->IsConflicting(llmqType, id, tx.GetHash());})
                ) {
            LogPrintf("CInstantSendManager::%s -- txid=%s: node.quorumSigningManager->IsConflicting returned true. id=%s\n", __func__,
    ...
    for (const auto i : irange::range(tx.vin.size())) {
        ...
        if (node.quorumSigningManager->AsyncSignIfMember(llmqType, id, tx.GetHash(), {}, fRetroactive)) {
...
bool CInstantSendManager::CheckCanLock(const COutPoint& outpoint, bool printDebug, const uint256& txHash, const Consensus::Params& params) const
{
    ...
    if (nTxAge < nInstantSendConfirmationsRequired && !node.chainLocksHandler->HasChainLock(pindexMined->nHeight, pindexMined->GetBlockHash())) {
        if (printDebug) {
...
void CInstantSendManager::TrySignInstantSendLock(const CTransaction& tx)
{
    ...
    for (auto& in : tx.vin) {
        auto id = ::SerializeHash(std::make_pair(INPUTLOCK_REQUESTID_PREFIX, in.prevout));
        if (!node.quorumSigningManager->HasRecoveredSig(llmqType, id, tx.GetHash())) {
            return;
        }
    }
    ...
    if (node.quorumSigningManager->HasRecoveredSigForId(llmqType, id)) {
        return;
    }
    ...
    node.quorumSigningManager->AsyncSignIfMember(llmqType, id, tx.GetHash());
}
...
std::unordered_set<uint256, StaticSaltedHasher> CInstantSendManager::ProcessPendingInstantSendLocks(const Consensus::LLMQType llmqType, int signOffset, const std::unordered_map<uint256, std::pair<NodeId, CInstantSendLockPtr>, StaticSaltedHasher>& pend, bool ban)
{
    ...
    for (const auto& p : pend) {
        ...
        if (node.quorumSigningManager->HasRecoveredSig(llmqType, id, islock->txid)) {
            alreadyVerified++;
            continue;
        }
        ...
        auto quorum = node.quorumSigningManager->SelectQuorumForSigning(llmqType, id, nSignHeight, signOffset);
        if (!quorum) {
    ...
    for (const auto& p : pend) {
        ...
        if (it != recSigs.end()) {
            ...
            if (!node.quorumSigningManager->HasRecoveredSigForId(llmqType, recSig->getId())) {
                LogPrint(BCLog::INSTANTSEND, "CInstantSendManager::%s -- txid=%s, islock=%s: passing reconstructed recSig to signing mgr, peer=%d\n", __func__,
                         islock->txid.ToString(), hash.ToString(), nodeId);
                node.quorumSigningManager->PushReconstructedRecoveredSig(recSig);
            }
        }
    }
...
void CInstantSendManager::ProcessInstantSendLock(NodeId from, const uint256& hash, const CInstantSendLockPtr& islock)
{
    ...
    if (tx && !hashBlock.IsNull()) {
        ...
        if (pindexMined != nullptr && node.chainLocksHandler->HasChainLock(pindexMined->nHeight, pindexMined->GetBlockHash())) {
...
void CInstantSendManager::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex, const std::vector<CTransactionRef>& vtxConflicted)
{
    ...
    if (masternodeSync.IsBlockchainSynced()) {
        for (const auto& tx : pblock->vtx) {
            ...
            if (!IsLocked(tx->GetHash()) && !node.chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
...
void CInstantSendManager::TruncateRecoveredSigsForInputs(const llmq::CInstantSendLock& islock)
{
    for (auto& in : islock.inputs) {
        ...
        node.quorumSigningManager->TruncateRecoveredSig(CLLMQUtils::GetInstantSendLLMQType(islock.IsDeterministic()), inputRequestId);
    }
}
...
void CInstantSendManager::HandleFullyConfirmedBlock(const CBlockIndex* pindex)
{
    ...
    for (const auto& p : removeISLocks) {
        ...
        // fully confirmed now
        node.quorumSigningManager->TruncateRecoveredSig(CLLMQUtils::GetInstantSendLLMQType(islock->IsDeterministic()), islock->GetRequestId());
    }
...
void CInstantSendManager::ResolveBlockConflicts(const uint256& islockHash, const llmq::CInstantSendLock& islock)
{
    ...
    for (const auto& p : conflicts) {
        auto pindex = p.first;
        if (node.chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {

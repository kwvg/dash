...
namespace llmq {
QuorumParticipant::QuorumParticipant(CBLSWorker& bls_worker, CDeterministicMNManager& dmnman, CDKGSessionManager& dkgsman, CQuorumManager& qman,
...
QuorumParticipant::~QuorumParticipant()
{
}
...
void QuorumParticipant::CheckQuorumConnections(CConnman& connman, const Consensus::LLMQParams& llmqParams, gsl::not_null<const CBlockIndex*> pindexNew) const
{
    if (m_mn_activeman) {
...
void QuorumParticipant::CheckQuorumConnectionsMn(CConnman& connman, const Consensus::LLMQParams& llmqParams, gsl::not_null<const CBlockIndex*> pindexNew) const
{
    ...
    for (const auto& quorum : lastQuorums) {
        if (utils::EnsureQuorumConnections(llmqParams, connman, m_dmnman, m_sporkman, m_qsnapman,
                                           m_dmnman.GetListAtChainTip(), quorum->m_quorum_base_block_index, proTxHash,
                                           /* is_masternode=*/true, m_quorums_watch)) {
...
MessageProcessingResult QuorumParticipant::ProcessEncryptedContribs(CNode& pfrom, CConnman& connman, bool request_limit_exceeded,
                                                                    ...
                                                                    const CBlockIndex* const block_index, std::string_view msg_type)
{
    if (msg_type == NetMsgType::QGETDATA) {
        if (request.GetDataMask() & CQuorumDataRequest::ENCRYPTED_CONTRIBUTIONS) {
            ...
            if (!m_dkgsman.GetEncryptedContributions(request.GetLLMQType(), block_index, quorum.qc->validMembers, request.GetProTxHash(), vecEncrypted)) {
...
void QuorumParticipant::StartDataRecoveryThread(CConnman& connman, gsl::not_null<const CBlockIndex*> pIndex, CQuorumCPtr pQuorum, uint16_t nDataMaskIn) const
{
    assert(m_mn_activeman);

...
void QuorumParticipant::CheckQuorumConnectionsWatchOnly(CConnman& connman, const Consensus::LLMQParams& llmqParams,
                                                        gsl::not_null<const CBlockIndex*> pindexNew) const
{
    auto lastQuorums = m_qman.ScanQuorums(llmqParams.type, pindexNew, (size_t)llmqParams.keepOldConnections);
    auto deletableQuorums = GetQuorumsToDelete(connman, llmqParams, pindexNew);
                                                                    ...
                                                                    const CBlockIndex* const block_index, std::string_view msg_type)
{
    if (msg_type == NetMsgType::QGETDATA) {
        if (request.GetDataMask() & CQuorumDataRequest::ENCRYPTED_CONTRIBUTIONS) {
            ...
            if (auto qdkgsman = m_qman.m_qdkgsman.load(std::memory_order_acquire); !qdkgsman || !qdkgsman->GetEncryptedContributions(request.GetLLMQType(), block_index, quorum.qc->validMembers, request.GetProTxHash(), vecEncrypted)) {
...
void QuorumParticipant::DataRecoveryThread(CConnman& connman, gsl::not_null<const CBlockIndex*> block_index, CQuorumCPtr pQuorum, uint16_t data_mask,

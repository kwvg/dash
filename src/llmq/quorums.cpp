                                                                 ...
                                                                 const CBlockIndex* const block_index, std::string_view msg_type)
{
    if (msg_type == NetMsgType::QGETDATA) {
        if (request.GetDataMask() & CQuorumDataRequest::ENCRYPTED_CONTRIBUTIONS) {
            ...
            if (auto qdkgsman = m_qdkgsman.load(std::memory_order_acquire); !qdkgsman || !qdkgsman->GetEncryptedContributions(request.GetLLMQType(), block_index, quorum.qc->validMembers, request.GetProTxHash(), vecEncrypted)) {
...
MessageProcessingResult CQuorumManager::ProcessMessage(CNode& pfrom, CConnman& connman, std::string_view msg_type, CDataStream& vRecv)
{
    if (msg_type == NetMsgType::QGETDATA) {
        ...
        auto ret = ProcessEncryptedContribs(pfrom, connman, request_limit_exceeded, ssResponseData, const_cast<CQuorum&>(*pQuorum), request, pQuorumBaseBlockIndex, msg_type);

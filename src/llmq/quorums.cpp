...
MessageProcessingResult CQuorumManager::ProcessEncryptedContribs(CNode& pfrom, CConnman& connman,
                                                                 ...
                                                                 std::string_view msg_type)
{
    if (msg_type == NetMsgType::QGETDATA) {
        if (request.GetDataMask() & CQuorumDataRequest::ENCRYPTED_CONTRIBUTIONS) {
            ...
            if (!m_qdkgsman ||
                !m_qdkgsman->GetEncryptedContributions(request.GetLLMQType(), block_index,
...
MessageProcessingResult CQuorumManager::ProcessMessage(CNode& pfrom, CConnman& connman, std::string_view msg_type, CDataStream& vRecv)
{
    if (msg_type == NetMsgType::QGETDATA) {
        ...
        auto ret = ProcessEncryptedContribs(pfrom, connman, request_limit_exceeded, ssResponseData,
                                            const_cast<CQuorum&>(*pQuorum), request, pQuorumBaseBlockIndex, msg_type);
    ...
    if (msg_type == NetMsgType::QDATA) {
        ...
        if (auto ret = ProcessEncryptedContribs(pfrom, connman, /*request_limit_exceeded=*/false, vRecv, *pQuorum,

    ...
    while (!interruptNet)
    {
        ...
        const auto getPendingQuorumNodes = [&]() EXCLUSIVE_LOCKS_REQUIRED(cs_vPendingMasternodes) {
            ...
            for (const auto& group : masternodeQuorumNodes) {
                for (const auto& proRegTxHash : group.second) {
                    ...
                    const auto& addr2 = dmn->pdmnState->addr.GetPrimaryService();
        ...
        auto getConnectToDmn = [&]() -> CDeterministicMNCPtr {
            ...
            if (!vPendingMasternodes.empty()) {
                ...
                if (dmn && !connectedNodes.count(dmn->pdmnState->addr.GetPrimaryService()) && !IsMasternodeOrDisconnectRequested(dmn->pdmnState->addr.GetPrimaryService())) {
                    LogPrint(BCLog::NET_NETCONN, "CConnman::%s -- opening pending masternode connection to %s, service=%s\n", _func_, dmn->proTxHash.ToString(), dmn->pdmnState->addr.GetPrimaryService().ToStringAddrPort());
                    return dmn;
                }
            }
        ...
        OpenMasternodeConnection(CAddress(connectToDmn->pdmnState->addr.GetPrimaryService(), NODE_NETWORK), /*use_v2transport=*/GetLocalServices() & NODE_P2P_V2, isProbe);
        // should be in the list now if connection was opened
        bool connected = ForNode(connectToDmn->pdmnState->addr.GetPrimaryService(), CConnman::AllNodes, [&](CNode* pnode) {
            if (pnode->fDisconnect) {
                return false;
            }
            return true;
        ...
        if (!connected) {
            LogPrint(BCLog::NET_NETCONN, "CConnman::%s -- connection failed for masternode  %s, service=%s\n", __func__, connectToDmn->proTxHash.ToString(), connectToDmn->pdmnState->addr.GetPrimaryService().ToStringAddrPort());

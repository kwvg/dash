    ...
    while (!interruptNet)
    {
        ...
        const auto getPendingQuorumNodes = [&]() EXCLUSIVE_LOCKS_REQUIRED(cs_vPendingMasternodes) {
            ...
            for (const auto& group : masternodeQuorumNodes) {
                for (const auto& proRegTxHash : group.second) {
                    ...
                    const auto& addr2 = dmn->pdmnState->netInfo.m_addr;
        ...
        auto getConnectToDmn = [&]() -> CDeterministicMNCPtr {
            ...
            if (!vPendingMasternodes.empty()) {
                ...
                if (dmn && !connectedNodes.count(dmn->pdmnState->netInfo.m_addr) && !IsMasternodeOrDisconnectRequested(dmn->pdmnState->netInfo.m_addr)) {
                    LogPrint(BCLog::NET_NETCONN, "CConnman::%s -- opening pending masternode connection to %s, service=%s\n", _func_, dmn->proTxHash.ToString(), dmn->pdmnState->netInfo.m_addr.ToStringAddrPort());
                    return dmn;
                }
            }
        ...
        bool connected = ForNode(connectToDmn->pdmnState->netInfo.m_addr, CConnman::AllNodes, [&](CNode* pnode) {
            if (pnode->fDisconnect) {
                return false;
            }
            return true;
        ...
        if (!connected) {
            LogPrint(BCLog::NET_NETCONN, "CConnman::%s -- connection failed for masternode  %s, service=%s\n", __func__, connectToDmn->proTxHash.ToString(), connectToDmn->pdmnState->netInfo.m_addr.ToStringAddrPort());

    ...
    if (msg_type == NetMsgType::INV) {
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                if (RejectIncomingTxs(pfrom) && NetMessageViolatesBlocksOnly(inv.GetCommand())) {
                ...
                if (!fAlreadyHave) {
                    if (RejectIncomingTxs(pfrom) && inv.type == MSG_ISDLOCK) {
                        if (pfrom.GetCommonVersion() <= ADDRV2_PROTO_VERSION) {
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        auto queueAndMaybePushInv = [this, pto, peer, &vInv, &msgMaker](const CInv& invIn) {
            auto inv_relay = peer->GetInvRelay();
            AssertLockHeld(inv_relay->m_tx_inventory_mutex);
            inv_relay->m_tx_inventory_known_filter.insert(invIn.hash);

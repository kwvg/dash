...
public:
    ...
    void forEachMN(bool only_valid, std::function<void(const MnEntryCPtr&)> cb) const override
    {
        m_list.ForEachMNShared(only_valid, [&cb](const auto& dmn) {
            cb(std::make_unique<const MnEntryImpl>(dmn));
        });
    }
...
public:
    ...
    {
        if (context().chainman) {
            if (const auto* pindexTip = WITH_LOCK(::cs_main, return context().chainman->ActiveChain().Tip())) {
                auto& chain_helper{context().chainman->ActiveChainstate().ChainHelper()};
                const auto pool = chain_helper.GetCreditPool(pindexTip);
                CreditPoolCounts ret;
                ret.m_locked = pool.locked;
                ret.m_limit = pool.currentLimit;
                if (pindexTip->pprev) {
                    ret.m_diff = pool.locked - chain_helper.GetCreditPool(pindexTip->pprev).locked;
                }
                return ret;
            }
        }
    ...
    std::vector<QuorumInfo> getQuorumStats() override
    {
        ...
        if (context().llmq_ctx && context().llmq_ctx->qman && context().chainman) {
            if (const auto* pindexTip = WITH_LOCK(::cs_main, return context().chainman->ActiveChain().Tip())) {
                for (const auto& type : llmq::GetEnabledQuorumTypes(*context().chainman, pindexTip)) {
                    if (const auto llmq_params = Params().GetLLMQ(type)) {
                        const auto quorums = context().llmq_ctx->qman->ScanQuorums(type, pindexTip, llmq_params->signingActiveQuorumCount);
                        double health = 0.0;

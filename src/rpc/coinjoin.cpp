...
static UniValue coinjoin(const JSONRPCRequest& request)
{
    ...
    auto clientman = ::coinJoinClientManagers->Get(*wallet);
    CHECK_NONFATAL(clientman != nullptr);
    ...
    if (request.params[0].get_str() == "start") {
        ...
        bool result = clientman->DoAutomaticDenominating(*node.connman, fee_estimator, mempool);
        return "Mixing " + (result ? "started successfully" : ("start failed: " + clientman->GetStatuses().original + ", will retry"));
    }

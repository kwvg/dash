...
static RPCArg GetRpcArg(const std::string& strParamName)
{
    static const std::map<std::string, RPCArg> mapParamHelp = {
        ...
        {"platformHTTPAddrs",
            {"platformHTTPAddrs", RPCArg::Type::ARR, RPCArg::Optional::NO,
                "Array of addresses in the form \"ADDR:PORT\" used by Platform for their HTTP API.\n"
                "Must be unique on the network. Can be set to an empty string, which will require a ProUpServTx afterwards.",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, ""},
                }}
        },
        {"platformHTTPAddrs_update",
            {"platformHTTPAddrs", RPCArg::Type::ARR, RPCArg::Optional::NO,
                "Array of addresses in the form \"ADDR:PORT\" used by Platform for their HTTP API.\n"
                "Must be unique on the network.",
                {
                                              ...
                                              const MnType mnType)
{
    ...
    CProRegTx ptx;
    ptx.nType = mnType;
    ptx.nVersion = ProTxVersion::GetMaxFromDeployment(WITH_LOCK(::cs_main, return chainman.ActiveChain().Tip()), /*is_basic_override=*/!use_legacy);
...
static RPCHelpMan protx_update_service_evo()
{
    ...
    return RPCHelpMan{
            ...
            HELP_REQUIRING_PASSPHRASE,
        {
            ...
            GetRpcArg("platformHTTPAddrs_update"),
            GetRpcArg("operatorPayoutAddress"),
            GetRpcArg("feeSourceAddress"),
        },
        RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    CProUpRevTx ptx;
    ptx.nVersion = ProTxVersion::GetMaxFromDeployment(WITH_LOCK(::cs_main, return chainman.ActiveChain().Tip()));

    ...
    if (input.isStr()) {
        if (!optional && input.get_str().empty()) {
            ...
            if (auto entryRet = ptx.netInfo->AddEntry(input.get_str()); entryRet != NetInfoStatus::Success) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Error setting ipAndPort[0] to '%s' (%s)", input.get_str(), NISToString(entryRet)));
            }
        }
        ...
        if (!optional && input.get_array().empty()) {
            ...
            for (size_t idx{0}; idx < entries.size(); idx++) {
                ...
                if (!entry.isStr()) {
                    throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid param for ipAndPort[%d], must be string", idx));
                }
                if (auto entryRet = ptx.netInfo->AddEntry(entry.get_str()); entryRet != NetInfoStatus::Success) {
                    throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Error setting ipAndPort[%d] to '%s' (%s)", idx, entry.get_str(), NISToString(entryRet)));
                }
            }
        }
    }

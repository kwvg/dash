...
static UniValue masternodelist(const JSONRPCRequest& request, const CChain& active_chain);
...
static UniValue masternode_winners(const JSONRPCRequest& request, const CChain& active_chain)
{
...
static UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (command == "masternodeconnect") {
        ...
        return masternode_winners(new_request, active_chain);
    } else if (command == "masternodelist") {
        return masternodelist(new_request, active_chain);
    } else {
        masternode_help();
    }
}
...
static UniValue masternodelist(const JSONRPCRequest& request, const CChain& active_chain)
{
    ...
    auto dmnToLastPaidTime = [&](auto& dmn) {
        ...
        LOCK(cs_main);
        const CBlockIndex* pindex = active_chain[dmn.pdmnState->nLastPaidHeight];
        return (int)pindex->nTime;
    };

    bool showRecentMnsOnly = strMode == "recent";
    bool showHPMNsOnly = strMode == "hpmn";
    int tipHeight = WITH_LOCK(cs_main, return active_chain.Tip()->nHeight);

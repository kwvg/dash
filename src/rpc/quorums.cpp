...
static UniValue quorum_list(const JSONRPCRequest& request, const CChain& active_chain, const LLMQContext& llmq_ctx)
{
    ...
    CBlockIndex* pindexTip = active_chain.Tip();
...
static UniValue quorum_list_extended(const JSONRPCRequest& request, const CChain& active_chain, const LLMQContext& llmq_ctx)
{
    ...
    CBlockIndex* pblockindex = nHeight != -1 ? active_chain[nHeight] : active_chain.Tip();
...
static UniValue quorum_dkgstatus(const JSONRPCRequest& request, const CChain& active_chain, const LLMQContext& llmq_ctx)
{
        ...
        for (const int quorumIndex : irange::range(quorums_num)) {
            ...
            if (fMasternodeMode) {
                ...
                if (quorumHeight <= tipHeight) {
                    const CBlockIndex* pQuorumBaseBlockIndex = active_chain[quorumHeight];
...
static UniValue quorum_memberof(const JSONRPCRequest& request, const CChain& active_chain, const LLMQContext& llmq_ctx)
{
    ...
    const CBlockIndex* pindexTip = active_chain.Tip();
...
static UniValue _quorum(const JSONRPCRequest& request)
{
    ...
    if (command == "quorumlist") {
        return quorum_list(new_request, active_chain, llmq_ctx);
    } else if (command == "quorumlistextended") {
        return quorum_list_extended(new_request, active_chain, llmq_ctx);
    } else if (command == "quoruminfo") {
        return quorum_info(new_request, llmq_ctx);
    } else if (command == "quorumdkgstatus") {
        return quorum_dkgstatus(new_request, active_chain, llmq_ctx);
    } else if (command == "quorummemberof") {
        return quorum_memberof(new_request, active_chain, llmq_ctx);
...
static UniValue verifychainlock(const JSONRPCRequest& request)
{
    ...
    if (request.params[2].isNull()) {
        ...
        if (nBlockHeight <= active_chain.Height()) {
            pIndex = active_chain[nBlockHeight];
        }
    }
...
static UniValue verifyislock(const JSONRPCRequest& request)
{
    ...
    {
        if (signHeight == -1) {
            pBlockIndex = active_chain.Tip();
        } else {
            pBlockIndex = active_chain[signHeight];
        }
    }

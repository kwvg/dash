...
static const std::vector<std::tuple<
    ...
    addr_vals{
        ...
        {{Purpose::PLATFORM_HTTP, "pg6mmjiyjmcrsslvykfwnntlaru7p5svn6y2ymmju6nubxndf4pscryd.onion:0"},
         NetInfoStatus::MaxLimit,
         NetInfoStatus::BadPort},
        // ExtNetInfo can store onion addresses
        {{Purpose::PLATFORM_HTTP, "pg6mmjiyjmcrsslvykfwnntlaru7p5svn6y2ymmju6nubxndf4pscryd.onion:8888"},
         NetInfoStatus::MaxLimit,
         NetInfoStatus::Success},
        // ExtNetInfo can store I2P addresses as long as it uses port 0
        {{Purpose::PLATFORM_HTTP, "udhdrtrcetjm5sxzskjyr5ztpeszydbh4dpl3pl4utgqqw2v4jna.b32.i2p:0"},
         NetInfoStatus::MaxLimit,
         NetInfoStatus::Success},
        // ExtNetInfo can store I2P addresses but non-zero ports are not allowed
        {{Purpose::PLATFORM_HTTP, "udhdrtrcetjm5sxzskjyr5ztpeszydbh4dpl3pl4utgqqw2v4jna.b32.i2p:8888"},
         NetInfoStatus::MaxLimit,
         NetInfoStatus::BadPort},
...
BOOST_AUTO_TEST_CASE(domainport_rules)
{
    static const std::vector<std::pair</*addr=*/std::string, /*retval=*/DomainPort::Status>> domain_vals{
        ...
        {"Loremipsumdolorsitametconsecteturadipiscingelitseddoeiusmodtempor"
         ...
         "iruredolorinreprehenderitinvoluptatevelitessecillumdoloreeufugiatnullapariat.ur",
         DomainPort::Status::BadLen},
    ...
    for (const auto& [addr, retval] : domain_vals) {
        ...
        if (retval != DomainPort::Status::Success) {
            ...
            BOOST_CHECK_EQUAL(netInfo.AddEntry(Purpose::PLATFORM_HTTP, service.ToStringAddrPort()),
...
BOOST_AUTO_TEST_CASE(extnetinfo_rules)
{
...

    {
        ...
        for (const auto purpose : {Purpose::CORE_P2P, Purpose::PLATFORM_P2P}) {
            BOOST_CHECK_EQUAL(netInfo.AddEntry(purpose, strprintf("[2606:4700:4700::1111]:%d", port)),
                              NetInfoStatus::BadInput);
        ...
        BOOST_CHECK_EQUAL(netInfo.AddEntry(Purpose::PLATFORM_HTTP, strprintf("[2606:4700:4700::1111]:%d", port)),
                          NetInfoStatus::Success);
...

    {
        ...
        ExtNetInfo netInfo;
        BOOST_CHECK_EQUAL(NISToString(netInfo.AddEntry(Purpose::PLATFORM_HTTP,
                                                       "[fc00:3344:5566:7788:9900:aabb:ccdd:eeff]:1234")),
                          NISToString(NetInfoStatus::Success));
        BOOST_CHECK_EQUAL(netInfo.Validate(), NetInfoStatus::Success);
        BOOST_CHECK(netInfo.HasEntries(Purpose::PLATFORM_HTTP));
        ValidateGetEntries(netInfo.GetEntries(), /*expected_size=*/1);

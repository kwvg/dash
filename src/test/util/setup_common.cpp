    ...
    : ChainTestingSetup(chainName, extra_args)
{
    ...
    m_node.chainman->InitializeChainstate(*m_node.mempool, llmq::chainLocksHandler, llmq::quorumInstantSendManager, llmq::quorumBlockProcessor, m_node.evodb);
    ...
    m_node.peer_logic = std::make_unique<PeerLogicValidation>(
        m_node.connman.get(), *m_node.addrman, m_node.banman.get(), *m_node.scheduler, *m_node.chainman, *m_node.mempool, m_node.llmq_ctx
    );
    {

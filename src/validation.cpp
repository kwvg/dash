    ...
    const std::string& warning_messages) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    AssertLockHeld(::cs_main);
    LogPrintf("%s%s: new best=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo) evodb_cache=%.1fMiB%s\n",
        prefix, func_name,
...
void CChainState::UpdateTip(const CBlockIndex* pindexNew)
{
    ...
    assert(std::addressof(::ChainstateActive()) == std::addressof(*this));
    UpdateTipLog(coins_tip, pindexNew, m_params, *m_evoDb, __func__, "", warningMessages);
}
                                                     ...
                                                     const std::optional<uint256>& snapshot_blockhash)
{
    ...
    to_modify.reset(new CChainState(mempool, m_blockman, *this, clhandler, isman, quorum_block_processor, evoDb, snapshot_blockhash));
        ...
        bool in_memory)
{
    ...
    auto snapshot_chainstate = WITH_LOCK(::cs_main, return std::make_unique<CChainState>(
            /* mempool */ nullptr, m_blockman, *this, this->ActiveChainstate().m_clhandler, this->ActiveChainstate().m_isman,

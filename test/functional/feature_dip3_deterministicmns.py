...
class DIP3Test(BitcoinTestFramework):
    ...
    def test_protx_update_service(self, mn):
        ...
        for node in self.nodes:
            ...
            assert_equal(protx_info['state']['addresses'][0], '127.0.0.2:%d' % mn.p2p_port)
            assert_equal(mn_list['%s-%d' % (mn.collateral_txid, mn.collateral_vout)]['addresses'][0], '127.0.0.2:%d' % mn.p2p_port)
